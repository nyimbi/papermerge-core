...
async def test_department_sovereignty_denied():
    ...
    service.db.get_active_policies = AsyncMock(return_value=[])
...
async def test_department_sovereignty_allowed_same_dept():
    ...
    session = AsyncMock()
    service = PolicyService(session)
    service.db.log_evaluation = AsyncMock()
...
async def test_department_permission_level():
    ...
    # Real engine with no policies — falls through to department_permissions check
